...
describe("lesson editor saving overlay", () => {
  it("uses delayed signature loader for long video save while keeping analytical fallback", () => {
    ...
    expect(source.includes("useDelayedLoading(")).toBe(true);
    expect(source.includes("SIGNATURE_VIDEO_SAVE_DELAY_MS")).toBe(true);
    expect(source.includes("<SignatureMathLoader")).toBe(true);
