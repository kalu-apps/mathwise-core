...
import { BookingSlotHoldOrchestrator } from "./bookings.slot-hold-orchestrator";
import {
  IDEMPOTENCY_TTL_SEC,
  LOCK_TTL_SEC,
  ensureId,
...
export class BookingsService implements OnModuleInit {
  ...
  private readonly slotHoldOrchestrator: BookingSlotHoldOrchestrator;
  ...
  constructor(
    ...
    this.slotHoldOrchestrator = new BookingSlotHoldOrchestrator({
      runtimeConfig: this.runtimeConfig,
      bookingsRepository: this.bookingsRepository,
      authRepository: this.authRepository,
      withLock: (key, operation) => this.withLock(key, operation),
      assertIdentityUsableForBooking: (actorUser) =>
        this.assertIdentityUsableForBooking(actorUser),
      syncBookingCapabilities: (booking) => this.syncBookingCapabilities(booking),
    });
  }
  ...
  async createSlotHold(params: {
    ...
    return this.slotHoldOrchestrator.createSlotHold(params);
  }

  async getSlotHoldStatus(params: {
    holdId: string;
  }): Promise<BookingSlotHoldStatusResponseDto> {
    return this.slotHoldOrchestrator.getSlotHoldStatus(params);
  }
  ...
  async confirmSlotHoldBooking(params: {
    ...
    return this.slotHoldOrchestrator.confirmSlotHoldBooking(params);
  }
